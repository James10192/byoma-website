...
export function BookingForm({ studio, range }: { studio: Doc<'studios'>; range: DateRange }) {
  ...
  return (
    <form onSubmit={onSubmit} noValidate>
      {/* Récapitulatif prix */}
      <div style={{ background: 'var(--gold-xlight)', border: '1px solid var(--gold-light)', borderRadius: 4, padding: 18, marginBottom: 20 }}>
        {range ? (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', color: 'var(--muted-2)' }}>
              <span>Arrivée</span>
              <strong style={{ color: 'var(--dark)' }}>{formatDate(range.checkIn)}</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', color: 'var(--muted-2)', marginTop: 6 }}>
              <span>Départ</span>
              <strong style={{ color: 'var(--dark)' }}>{formatDate(range.checkOut)}</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', color: 'var(--muted-2)', marginTop: 6 }}>
              <span>{formatXOF(studio.pricePerDay)} × {days} {days > 1 ? 'nuits' : 'nuit'}</span>
              <span />
            </div>
            <div style={{ height: 1, background: 'var(--gold-light)', margin: '12px 0' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <span style={{ fontWeight: 600, color: 'var(--dark)' }}>Total estimé</span>
              <span className="font-display" style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--gold-dark)' }}>{formatXOF(total)}</span>
            </div>
          </>
        ) : (
      ...
      {error && (
        <div role="alert" style={{ marginTop: 16, display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--error-light)', border: '1px solid #F0C9C4', borderRadius: 4, padding: '12px 14px', color: 'var(--error)', fontSize: '0.875rem' }}>
...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <label style={{ display: 'block' }}>
      <span style={{ display: 'block', fontSize: '0.8125rem', fontWeight: 600, color: 'var(--muted-2)', marginBottom: 6 }}>{label}</span>
      {children}
